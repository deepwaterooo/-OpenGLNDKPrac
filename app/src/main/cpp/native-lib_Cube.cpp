...
/*     "}"; */
const char VERTEX_SHADER[] = // 这个顶点着着器字符数组有点儿问题，compile不了
    "#version 300 es\n"
...
bool init() {
    programCube = createProgram(VERTEX_SHADER, FRAGMENT_SHADER);
    if (!programCube)
        return false;
...
extern "C" {
    JNIEXPORT jboolean JNICALL Java_com_me_opengl_Cube_initC(JNIEnv *env, jclass type);
    JNIEXPORT void JNICALL Java_com_me_opengl_Cube_resizeC(JNIEnv *env, jclass type, jint width, jint height);
    JNIEXPORT void JNICALL Java_com_me_opengl_Cube_renderC(JNIEnv *env, jclass type);
    JNIEXPORT void JNICALL Java_com_me_opengl_Cube_destroyC(JNIEnv *env, jclass type);
}
jboolean
Java_com_me_opengl_Cube_initC(JNIEnv *env, jclass type) {
    if (init())return JNI_TRUE;
    return JNI_FALSE;
}
void
Java_com_me_opengl_Cube_resizeC(JNIEnv *env, jclass type,
...
void
Java_com_me_opengl_Cube_renderC(JNIEnv *env, jclass type) {
    timespec now;
...
void Java_com_me_opengl_Cube_destroyC(JNIEnv *env, jclass type) {
